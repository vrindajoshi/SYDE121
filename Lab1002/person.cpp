...
Person::Person(const string &newname, const int newid)
  ...
  //   cout << "executing Person::Person(const string &newname, const int
  //   newid)"
  //        << endl;
  dateofbirth = Date(22, 7, 1900);
}

Person::Person(const string &newname, const int newid, const int day,
               const int month, const int year)
    : dateofbirth(day, month, year), name(newname), id(newid) {
  //   cout << "executing Person::Person(const string &newname,const int
  //   newid,"; cout << " const int day, const int month, const int year)" <<
  //   endl;
}
                                ...
                                name( newname ), id( newid )
{
     dateofbirth = Date(day,month,year);
        cout << "executing Person::Person(const string &newname,const int
newidconst,"; cout << "const int day,const int month,const int year) :
...
void Person::set_name(const string &newname) { name = newname; }

int Person::get_id() const { return id; }

void Person::set_id(const int newid) {
  id = newid;
  return;
}

Date Person::get_dateofbirth() const { return dateofbirth; }
