...
Student::Student(const int newgradyear) : gradyear(newgradyear), Person() {
  // cout << "executing Student::Student( int newgradyear ) : gradyear( 0 )" <<
  // endl;
}

Student::Student(const string &newname, const int newid, const int newgradyear)
    : Person(newname, newid), gradyear(newgradyear) {
  // cout << "executing Student::Student(const string &newname,const int
  // newgradyear) : "
...
void Student::change_name(const string &newname) {
  // name = newname;    this does not compile since 'name' is private in Person
  // class

  set_name(
      newname); // this command works since 'set_name' is public in the
                //   person class
                // Note: 'change_name' is actually unnecessary since
                //   Student objects will have access to the public
                //   'set_name' Person member fn
                // Note: can also equivablently all Person::set_name(newname);
}

Student::~Student() {}

int Student::get_gradyear() const { return gradyear; }

void Student::set_gradyear(const int newgradyear) { gradyear = newgradyear; }
